...
export const Allrounder: React.FC<PlayersListProps> = ({ closeModal }) => {
    ...
    const { data } = useQuery("allrounder", async () => {
        const response = await fetch(import.meta.env.VITE_BACKEND_URI + "players/getPlayersByType/All Rounder");
        if (!response.ok) {

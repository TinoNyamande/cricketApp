...
export const Bowlers: React.FC<PlayersListProps> = ({ closeModal }) => {
    ...
    const { data } = useQuery("bowlers", async () => {
        const response = await fetch(import.meta.env.VITE_BACKEND_URI + "players/getPlayersByType/Bowler");
        if (!response.ok) {
